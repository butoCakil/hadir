...
$fileLaporan = null;
$fileDokumentasi = "https://hadir.masbendz.com/data/Panduan Presensi PKL via WA.pdf";
...
if (preg_match('/^set\s+(\d{4})\s+([\d\s\+\-]+)/i', strtolower($message), $matches)) {
    ...
    $sendmsg = "📄 Panduan Laporan PKL saat ini *belum tersedia*.\nSilakan tunggu informasi selanjutnya dari Admin atau Pembimbing.\n\nTerima kasih atas kesabarannya 🙏";
    ...
    if (file_exists($penangguhanFile)) {
        ...
        if ($nis) {
            ...
            if (preg_match('/^(x{1,2}i)(at|dkv|te)(\d+)$/', $input, $matches)) {
        ...
        if (is_numeric($target)) {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        $row = mysqli_fetch_assoc($result);
                        $sendmsg = "✅ *Nomor ditemukan!*\n\n📱 *Nomor:* $target\n👤 *Nama:* {$row['nama']}\n🆔 *NIP:* {$row['nip']}\n\n📌 Data ini terdaftar di sistem presensi sebagai Pembimbing PKL 2025.";
                    } else {
    ...
    $sendmsg = "Panduan Penggunaan ChatBot Presensi PKL - 2025 SMKN Bansari";
        ...
        if (empty($sendmsg)) {
            ...
            if ($daNomorTerdaftar['type'] === 'siswa') {
                ...
                $sendmsg = "👋 Selamat datang, *{$daNomorTerdaftar['nama']}*!\n\n"
                    . "📌 Nomor ini terdaftar sebagai *Pembimbing PKL SMKN Bansari 2025*.\n\n"
    ...
    elseif (!$isInAdminSession) {
        ...
        if (empty($sendmsg)) {
            ...
            $default_msgs = [
                "📌 Ketik `info` untuk info layanan.\n📞 Balas `admin` atau `7` untuk hubungi admin.",
                "✅ Untuk daftar layanan, ketik `info`.\n👨‍💼 Untuk bantuan admin, ketik `admin` atau `7`.",
                "📖 Balas `info` untuk panduan.\n🆘 Butuh bantuan? Balas `admin` atau `7`.",
                "🚫 Pesan tanpa format tidak diproses. Gunakan `info` atau `admin`."
            ];
